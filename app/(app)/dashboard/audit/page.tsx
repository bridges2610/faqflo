...
/*
  ...
  `?purchased=get_cited` is set by the checkout return page. The workspace
  strips it on arrival — see the note there about why a refresh must not be able
  to start a second crawl.
...
export default async function AuditPage({
  ...
  searchParams: Promise<{ purchased?: string; view?: string }>;
}) {
  const { purchased, view } = await searchParams;

  return (
    <AuditWorkspace
      justPurchased={purchased === 'get_cited'}
